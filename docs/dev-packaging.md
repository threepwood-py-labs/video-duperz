...
```powershell
python scripts/windows/package_release.py --release-tag v0.1.6 --target-arch windows-x64
```
...
```powershell
python scripts/windows/package_onefile.py --release-tag v0.1.6 --target-arch windows-x64
```
...
```powershell
hatch run package:package-release --release-tag v0.1.6 --target-arch windows-x64
hatch run package:onefile
hatch run package:package-onefile --release-tag v0.1.6 --target-arch windows-x64
hatch run package:checksums
hatch run package:verify-assets --release-tag v0.1.6
```
...
- staged portable release:
  - `build/release/video-duperz-windows-x64-v0.1.6`
- release zip:
  - `build/release/video-duperz-windows-x64-v0.1.6.zip`
- public onefile executable:
  - `build/release/video-duperz-v0.1.6-windows-x64.exe`
...
3. Push the matching `v0.1.6` tag once the result is correct.
...
- release assets contain:
  - `video-duperz-v0.1.6-windows-x64.exe`
  - `video-duperz-v0.1.6-windows-arm64.exe`
  - `video-duperz-windows-x64-v0.1.6.zip`
  - `video-duperz-windows-arm64-v0.1.6.zip`
  - `SHA256SUMS.txt`
